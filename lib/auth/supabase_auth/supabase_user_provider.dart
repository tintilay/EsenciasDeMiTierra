...
class EsenciasDeMiTierraSupabaseUser extends BaseAuthUser {
  EsenciasDeMiTierraSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> esenciasDeMiTierraSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = EsenciasDeMiTierraSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
